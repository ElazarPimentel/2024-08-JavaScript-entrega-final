...
export const mostrarError = (elemento: HTMLElement, mensaje: string): void => {
    let error = elemento.nextElementSibling as HTMLElement | null;
...
export const validarNombre = (nombre: string): boolean => /^[a-zA-Z\s]{2,25}$/.test(nombre);

// Validar números de teléfono: permite de 7 a 20 caracteres, incluyendo números y símbolos +, -, (, ), y espacios
export const validarTelefono = (telefono: string): boolean => /^[0-9+\-().\s]{7,20}$/.test(telefono);

// Validar el número de mascotas: permite un dígito único, del 1 al 3
export const validarNumeroMascotas = (num: string): boolean => /^[1-3]$/.test(num);
...
export const validarDiaAbierto = (fecha: string): boolean => {
    const dia = new Date(fecha + "T00:00:00").getDay();
    return dia >= 1 && dia <= 5;
};

// Validar la hora del turno, asegurando que esté dentro del horario laboral y considerando el número de mascotas
export const validarHora = (fecha: string, hora: string, horarios: Horario, numeroDeMascotas: string): boolean => {
    const dia = new Date(fecha + "T00:00:00").getDay();
...
export const validarEdadMascota = (edad: string): boolean => {
